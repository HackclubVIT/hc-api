...
async function main() {
  ...
  const recruitment = (Array.isArray(json.recruitmentApplications) ? json.recruitmentApplications : []).map((a) => ({
    ...
    recruitmentId: a.recruitmentId ?? 'recruitment-2026',
    name: a.name ?? 'Applicant',
    registerNumber: a.registerNumber ?? `REG${a.id}`,
    email: a.email ?? `applicant${a.id}@vitstudent.ac.in`,
    phoneNumber: a.phoneNumber ?? '',
    domain: a.domain ?? a.firstPreference ?? 'Technical',
    firstPreference: a.firstPreference ?? a.domain ?? 'Technical',
    secondPreference: a.secondPreference ?? null,
    firstPrefReason: a.firstPrefReason ?? a.whyJoin ?? '',
    secondPrefReason: a.secondPrefReason ?? '',
    yearOfStudy: a.yearOfStudy ?? '1st Year',
    github: a.github ?? '',
    linkedin: a.linkedin ?? '',
    ...
    whyJoin: a.whyJoin ?? a.firstPrefReason ?? '',
    projectDetails: a.projectDetails ?? a.sevenDaysBuild ?? '',
